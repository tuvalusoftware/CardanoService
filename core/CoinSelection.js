...
const CoinSelection = {
  ...
  randomImprove: (inputs, outputs, limit, preset = []) => {
    ...
    let amount = CardanoWasm.Value.new(CardanoWasm.BigNum.from_str("0"));
...
const cloneUTxOList = (utxoList) =>
  utxoList.map((utxo) =>
    CardanoWasm.TransactionUnspentOutput.from_bytes(utxo.to_bytes())
  );
 ...
 */
const cloneValue = (value) => CardanoWasm.Value.from_bytes(value.to_bytes());
