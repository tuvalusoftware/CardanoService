...
const fuixlabsProvider = {
  fetchAssetMetadata: async (unit: string) => {
    ...
    try {
      ...
      const asset: any = data.find((d: any) => d.asset_name === assetName && d.policy_id === policyId);
      const metadata: AssetMetadata = asset?.["minting_tx_metadata"]?.["721"]?.[policyId]?.[assetName!] || {};
      return metadata;
    } catch (error) {
      return {};
    }
